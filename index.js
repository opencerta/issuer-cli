...
cli
  ...
  .requiredOption('--issuer <certFile>', 'Certificate to sign with - Its publicKey must match input key')
  .action(csr.sign);

// Verifiable credentials
const vcCommand = cli.command('vc').description('Manage Health Certificates (verifiable credentials)');

vcCommand
  .command('create <healthCertFile>')
  .requiredOption('--type <Immunization|DiagnosticReport>', 'Health Certificate type')
...
vcCommand
  ...
  .requiredOption('--photo <file>', 'Add patient pictures', preprocess.concat, [])
